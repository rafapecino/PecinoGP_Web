        ...
        <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden">
          ...
          >
            <motion.div variants={itemVariants} className="flex items-center justify-center gap-2 mb-6">
              ...
              <span className="text-red-500 font-black uppercase tracking-[0.4em] text-[10px]">Próxima Cita</span>
            </motion.div>

            <motion.div variants={itemVariants} className="mb-4">
              <span className="bg-red-600 text-white font-black italic px-4 py-1 text-sm rounded-sm">ROUND {nextRace.round}</span>
            </motion.div>
            ...
            >
              {nextRace.gp.split(' ').slice(2).join(' ').toUpperCase()} <span className="text-red-600">GP</span>
            </motion.h1>
            
            <motion.div variants={itemVariants} className="flex flex-wrap items-center justify-center gap-6 md:gap-10 text-white/80 font-bold italic">
              <div className="flex items-center gap-2">
                <Calendar className="text-red-600" size={20} />
                <span className="tracking-widest uppercase text-sm">{nextRace.dates}</span>
              </div>
              <div className="flex items-center gap-2">
                <MapPin className="text-red-600" size={20} />
                <span className="tracking-widest uppercase text-sm">{nextRace.circuit}</span>
              </div>
            </motion.div>
          </motion.div>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 px-4">
              <div>
                <motion.div initial={{ x: -20, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-red-500 font-black uppercase tracking-[0.2em] text-[10px]">Temporada 2026</span>
                </motion.div>
                <h2 className="text-4xl md:text-6xl font-black text-white italic tracking-tighter">CALENDARIO <span className="text-red-600">COMPLETO</span></h2>
              </div>
            </div>
