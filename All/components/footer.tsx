...
export function Footer() {
  return (
    <footer className="px-4 sm:px-6 lg:px-8 pt-16 pb-10 border-t border-border bg-secondary">
      <div className="max-w-7xl mx-auto">
        {/* Top: logo + main nav */}
        <div className="flex flex-col items-center text-center">
          <Logo size="sm" />
          <nav className="flex justify-center gap-x-6 gap-y-3 flex-wrap mt-4 mb-8 text-muted-foreground">
          ...
          <div className="flex justify-center gap-6 mb-8">
            <Link href="https://www.youtube.com/@pecinogp" aria-label="YouTube" className="text-muted-foreground hover:text-primary transition-colors">
              <svg className="w-6 h-6" viewBox="0 0 28 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-6 text-center md:text-left">
          <div>
            <p className="text-muted-foreground text-sm">
              &copy; {new Date().getFullYear()} PecinoGP — MPC Network SL. Todos los derechos reservados.
            </p>
          ...
          <nav
            ...
            className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-xs text-muted-foreground/70"
          >
            ...
            <span aria-hidden className="text-muted-foreground/30">·</span>
            <ManageCookiesLink className="hover:text-primary transition-colors cursor-pointer" />
          </nav>
        </div>
      </div>
    </footer>
